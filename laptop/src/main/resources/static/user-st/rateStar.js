$(document).ready(function () {
    ...
    function rate(rate, average, text) {
        ...
        let starAverage = '<h6 style="margin-top: -7px">Trung bình: ' + text + '</h6>';
        if (average < 3) {
                ...
                if (message !== null) {
                    ...
                    if ($('.ajs-message').length < 3) {
                        ...
                        let html = '   <div class="media">\n' +
                            '                    <a class="pull-left" href="#"><img class="media-object" style="width: 50px; height: 50px" src="https://bootdey.com/img/Content/avatar/avatar1.png" alt=""></a>\n' +
        ...
        $.ajax(
            {
                ...
                data: data,
                dataType: 'json',
                url: origin + "/api/showComment/" + pageNumber,
                success: function (result) {
                    $('#totalComment').html('<h3>Bình luận(' + result.totalElements + ')</h3>')
                    var htmll = '';
                    if (parseInt(result.totalElements) === 0) {
                    ...
                    if (result.pageable.pageNumber === result.totalPages - 1) {
                        $.each(result.content, function (key, obj) {
                            htmll += '   <div class="media">\n' +
                                '                    <a class="pull-left" href="#"><img class="media-object" style="width: 50px; height: 50px; display: flex;object-fit: cover;" src="' + obj.account.imgUrl + '" alt=""></a>\n' +
                                '                    <div class="media-body">\n' +
