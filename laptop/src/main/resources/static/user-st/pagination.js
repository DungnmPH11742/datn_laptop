...
(function (factory) {
    ...
    Pagination.prototype = {
        ...
        render: function () {
            ...
            $target.empty();
            // 上一页
            $target.append('<li class="page-item"><a   style="cursor: pointer" herf="javascript:void(0)" tabindex="-1" data-page="prev">' + options.prev + '</a></li>');
            var page = this.getPages();
            if (page.start > 1) {
                $target.append('<li class="page-item"><a  style="cursor: pointer" herf="javascript:void(0)" data-v-82963a40="" tabindex="0" class="page-link" data-page="' + 1 + '">' + 1 + '</a></li>');
                $target.append('<li class="page-item"><a  style="cursor: pointer" data-v-82963a40="" tabindex="0" class="page-link" >...</a></li>');

            }
            // 生成中间页码
            for (var i = page.start; i <= page.end; i++) {
                $target.append('<li class="page-item"><a  style="cursor: pointer" herf="javascript:void(0)" tabindex="0" class="page-link" data-page="' + i + '">' + i + '</a></li>');
            }
            if (page.end < Math.ceil(options.total / options.length)) {
                $target.append('<li class="page-item"><a  style="cursor: pointer" data-v-82963a40="" tabindex="0" class="page-link" >...</a></li>');
                $target.append('<li class="page-item"><a  style="cursor: pointer" herf="javascript:void(0)" tabindex="0" class="page-link" data-page="' + Math.ceil(options.total / options.length) + '">' + Math.ceil(options.total / options.length) + '</a></li>');
            }
            // 下一页
            $target.append('<li><a  style="cursor: pointer" herf="javascript:void(0)" tabindex="0"  data-page="next">' + options.next + '</a></li>');
            // 设置当前页样式
            $target.find('li>a[data-page="' + options.current + '"]').parent().addClass('page-item active');
            // 设置上一页样式
            if (options.current <= 1) {
                $target.find('li>a[data-page="prev"]').parent().addClass("page-item disabled");
            }
            // 设置下一页样式
            if (options.current >= Math.ceil(options.total / options.length)) {
                $target.find('li>a[data-page="next"]').parent().addClass("page-item disabled");
            }
        },
        /**
         * 根据插件的参数获取分页页码渲染数据
        ...
        onClick: function () {
            ...
            $target.on('click', '>li>a[data-page]', function (e) {
                if ($(this).parent().hasClass('page-item disabled') || $(this).parent().hasClass('page-item active')) {
                    return
                }
